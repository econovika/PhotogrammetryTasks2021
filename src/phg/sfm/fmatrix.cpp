...
namespace {
    ...
    cv::Matx33d getNormalizeTransform(const std::vector<cv::Vec2d> &m)
    {
        ...
        for (auto& pt : m) {
            double dist = std::sqrt(std::pow(pt[0] - xMean, 2) + std::pow(pt[1] - yMean, 2));
        ...
        cv::Matx33d normMat;
        double diagEl = std::sqrt(2) / xyNorm;
        double el13 = - std::sqrt(2) * xMean / xyNorm;
        double el23 = - std::sqrt(2) * xMean / xyNorm;

        normMat << diagEl, 0, el13,
                   0, diagEl, el23,
                   0,      0,    1;
